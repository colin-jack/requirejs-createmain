...
module.exports = (function() {
	...
	MainFileGenerator.prototype.generate = function(allDone) {
        ...
        this.recursivelyProcessContents(this.directoryToScan, allDone);
	};

	MainFileGenerator.prototype.recursivelyProcessContents = function(directoryToScan, done) {
        var that = this;

        var processFile = function(err, file) {
            if (err) {
                return this.writeErrorAndComplete(err, done);
            }

            that.recursivelyStoreFileDetails(file, directoryToScan);
        }

        fs.readdir(this.directoryToScan, this.process);
    };

    MainFileGenerator.prototype.respondToNoFilesOrDirectoriesFound = function() {
    ...
    MainFileGenerator.prototype.processFileOrDirectoryStats = function(err, fileOrDirectoryStats) {
        ...
        {
            this.requiredFilesStore.saveFileDetails(fileOrDirectory, fullPathToFileOrDirectory);
            done();
        }
    };
    ...
    MainFileGenerator.prototype.recursivelyStoreFileDetails = function(fileOrDirectory, parentDirectory, done) {
        ...
        fs.stat(fullPathToFileOrDirectory, this.processFileOrDirectoryStats);
    };
