...
module.exports = (function() {
	...
	MainFileGenerator.prototype.recursivelyProcessContents = function(directory, done) {
        ...
        var processFileOrDirectory = function(file, callback) {
            that.recursivelyStoreFileDetails(file, directory, callback);
        }

       var processDirectoryContents = function(err, files) {
            if (err) {
            ...
            async.forEach(files, processFileOrDirectory, function(err) {
                done(err);
            });
        }

        fs.readdir(directory, processDirectoryContents);
    };
